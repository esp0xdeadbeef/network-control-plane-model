...
let
  ...
  portBinding =
    if hasAttr uplinkName portBindings.byUplink then portBindings.byUplink.${uplinkName} else null;
  ...
  validatedHostUplink =
    if realizedTarget then
      if builtins.elem uplinkName overlayNames then
        ...
        failInventory "inventory.deployment.hosts.${targetHostName}.uplinks" "${uplinkPath} on realized target '${targetId}' requires explicit host uplink mapping in inventory.deployment.hosts.${targetHostName}.uplinks"
      else
        builtins.seq (requireExplicitHostUplinkAddressing {
      ...
      resolvedHostUplink;
  baseRoutes = {
    ipv4 = builtins.map (dst: explicitUplinkRoute 4 dst) (
      if uplinkAttrs ? ipv4 then requireStringList "${uplinkPath}.ipv4" uplinkAttrs.ipv4 else [ ]
    );
    ipv6 = builtins.map (dst: explicitUplinkRoute 6 dst) (
      if uplinkAttrs ? ipv6 then requireStringList "${uplinkPath}.ipv6" uplinkAttrs.ipv6 else [ ]
    );
  };
  uplinkStaticRoutes =
    if hasAttr uplinkName uplinkRouting then
      staticUplinkRoutes uplinkRouting.${uplinkName}
    else
      {
        ipv4 = [ ];
        ipv6 = [ ];
...
in
builtins.seq _requiredPortBinding {
  name = uplinkName;
  inherit value;
}
