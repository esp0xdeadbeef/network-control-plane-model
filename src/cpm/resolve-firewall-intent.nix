...
let
  ...
  siteRelations =
    if builtins.isList (communicationContract.relations or null) then communicationContract.relations else listOrEmpty (communicationContract.allowedRelations or null);
          ...
          if (entry.target.role or null) == "core" then
            {
              ...
              value = buildNat { inherit siteAttrs overlayNames; inherit (entry) interfaceRecords target; };
            }
          else
            null)
        targetEntries);
    ...
    builtins.filter
      ...
      (map
        (entry:
          ...
          in
          if value == null then null else { name = entry.targetName; inherit value; })
        targetEntries);
