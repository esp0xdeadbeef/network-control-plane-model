...
let
  ...
  relationId =
    ...
    if isNonEmptyString (relation.id or null) then
      relation.id
    else if isNonEmptyString (relation.name or null) then
      relation.name
    else
      null;
  ...
  stripCidr =
    ...
    if !isNonEmptyString value then
      ...
      builtins.head (builtins.filter builtins.isString (builtins.split "/" value));

  p2pPeer4 =
    cidr:
    let
      parts =
        if builtins.isString cidr then builtins.filter builtins.isString (builtins.split "/" cidr) else [ ];
  ...
  serviceIndex =
    ...
    builtins.listToAttrs (
      map
        ...
        (
          builtins.filter (service: builtins.isAttrs service && isNonEmptyString (service.name or null)) (
            listOrEmpty services
          )
        )
    );

  targetAccessNodesFor =
    policyEndpointBindings: service:
    let
  ...
  endpointAddresses4 = endpoint: uniqueStrings (listOrEmpty ((attrsOrEmpty endpoint).ipv4 or null));

  endpointAddresses6 = endpoint: uniqueStrings (listOrEmpty ((attrsOrEmpty endpoint).ipv6 or null));
    ...
    let
      ...
      identifier =
        if parsed == null then
          ...
          builtins.genList (index: if index < 4 then 0 else builtins.elemAt parsed index) 8;
    in
    if identifier == null then null else ipam.renderIPv6 identifier;

  protectedRuntimePrefix =
    context: tenant: routedPrefixesByTenant:
    let
...
let
  ...
  publicIngressRelations = builtins.filter (
    ...
    && (
      ...
      in
      (mode == "napt" && (family == null || family == "ipv4")) || (mode == "none" && family == "ipv6")
    )
  ) relations;
  ...
  buildRecord =
    ...
    let
      ...
      relationUplinks =
        ...
        in
        if (endpoint.kind or null) != "external" then
          [ ]
        else
      ...
      declaredTargetEndpoint = authority.targetEndpoint or null;
      endpointCandidates = builtins.filter (
        endpoint:
        family == 6
        || !isNonEmptyString declaredTargetEndpoint
        || ((attrsOrEmpty endpoint).name or null) == declaredTargetEndpoint
      ) (listOrEmpty (service.providerEndpoints or null));
      endpoint = requireOne "relation '${toString id}' target endpoint" endpointCandidates;
      targetEndpoint = (attrsOrEmpty endpoint).name or null;
      ...
      targetAccessNode = requireOne "relation '${toString id}' target access node" (
      ...
      internalNextHop =
        if family == 6 then
          ...
          p2pPeer4 (internalEgress.addr4 or null);
      ...
      providerTenant =
        if family == 6 then
          requireOne "relation '${toString id}' provider tenant" (
            listOrEmpty (service.providerTenants or null)
          )
        else
          null;
      ...
      runtimePrefix =
        if family != 6 then
          ...
          protectedRuntimePrefix "relation '${toString id}' protected runtime prefix" providerTenant
            routedPrefixesByTenant;
      runtimeDestination =
        if family != 6 then
          null
      ...
      complete =
        ...
        && (
          family != 6 || !isNonEmptyString declaredPublicSurface || declaredPublicSurface == publicSurface
        )
        && (
          family != 6 || !isNonEmptyString declaredTargetEndpoint || declaredTargetEndpoint == targetEndpoint
        )
