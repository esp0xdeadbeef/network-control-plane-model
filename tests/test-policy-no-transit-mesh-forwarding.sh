#!/usr/bin/env bash
# GAMP-ID: SMT-CPM-POLICY-NO-TRANSIT-MESH-001
...
jq -r '
  ...
  | . as $target
  | (
      if ($target.value.forwardingIntent.mode // "") == "explicit-transit-mesh-forwarding" then
        "!!!! " + $enterprise.key + "." + $site.key
        + " policy=" + $target.key
        + " CPM policy firewall still uses transit-mesh mode"
      else empty end
    ),
    (
      ...
      | select(
          (.value.action // "") == "accept"
          and ((.value.relationId // .value.policyId // .value.sourceRelationId // "") == "")
        )
      | "!!!! " + $enterprise.key + "." + $site.key
        + " policy=" + $target.key
        + " CPM emitted unscoped policy accept without modeled policy id: "
...
echo "PASS policy-no-transit-mesh-forwarding"
