...
345 src/cpm/resolve-firewall-intent.nix | state=watch | reason=split policy deny contract assembly and per-family NAT target assembly from forwarding-entry wiring before adding more firewall behavior
...
- state=still-broken | target=S88 policy-router firewall projection | reason=`tests/test-s88-router-roles-have-firewall-policy.sh` proves several network-labs examples emit `explicit-policy-forwarding` policy routers with zero `forwardingIntent.rules`; either CPM is not projecting upstream allowed communication policy into role-local rules, or the upstream model is not exposing the policy surface CPM needs. This must fail before renderer tests because renderers must not invent policy from interface names.
